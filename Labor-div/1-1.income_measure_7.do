...
** Clasificación de ingresos en salaried y non-salaried para ocupación principal
...
local min_wage_y_ppp = ln(min_wage_y_ppp) // Minimum wage in 2022
local min_wage_h_ppp = ln(min_wage_h_ppp) // Minimum wage in 2022
/* gen min_wage_ppp=ln(117304 / ppp) // Minimum wage in 2022 */

/* keep `id_vars' codigoMiembro1  wage_* salario* boni* beneficios* totalHoras* agri_income livestock_income deriva_income business_income productivity is_worker2 total_* nonsalary_income* */
drop _m
...
/* br nworker total_income_y produc_y total_income_y_ppp total_income_h_ppp produc_y_ppp produc_h_ppp wage_y_ppp wage_h_ppp log_total_income_y_ppp log_total_income_h_ppp log_produc_y_ppp log_produc_h_ppp log_wage_y_ppp log_wage_h_ppp */

	keep if q1_03_edad>=16 & q1_03_edad<=70 // edad trabajar
	keep if oci==1 //ocupados
 /* br interview__key log_produc_h_ppp log_produc_y_ppp total_hr total_hr_impu total_income_y wage_y nonsalary_y salarioPrincipal */

save "${gdData_7L}/Produc_est1-1.dta", replace

drop if log_total_income_y_ppp<4
drop if log_produc_y_ppp<4
// Summarize the variables to check the results
